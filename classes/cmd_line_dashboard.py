...
def cmd_line_user_dashboard(user):

	items = user_controller.get_user_items(user.user_id)
	...
	while True:
		...
		if first_choice == "1":
			...
			companies = user_controller.get_user_companies(user.user_id)
			if not companies:
			...
			name = input("\nEnter company name: ")
			company_controller.add_company(name, user.user_id)
